...
const refreshAccessToken = asyncHandler(async (req, res) => {
    try {
        ...
        throw new ApiError(500, error?.message || "Something wen wrong");
    }
});
...
const updateAccountDetails = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, user, "Accunt details updated successfully")
        );
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    if (!channel?.length) throw new ApiError(400, "channel does not exsist");
